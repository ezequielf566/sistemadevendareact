...
import { Plus, Search, Calendar, Phone, MessageCircle, Filter, Trash2, AlertTriangle, X, Check, Wallet } from 'lucide-react';
...
const Clients: React.FC = () => {
  ...
  const handleBillClient = (client: Client) => {
    const settings = getSettings();
    const dueDate = calculateDueDate(new Date()).toISOString();
    
    const link = generateBillingLink(client.phone, {
    ...
    <div className="h-full flex flex-col relative">
        ...
        <div className="flex gap-2 w-full md:w-auto">
          <div className="flex bg-white/5 p-1 rounded-xl border border-white/10">
            <button 
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${filter === 'debt' ? 'bg-amber-500/20 text-amber-400' : 'text-gray-400 hover:text-white'}`}
            >
              Com Débito
            </button>
            <button 
              onClick={() => setFilter('all')}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${filter === 'all' ? 'bg-emerald-500/20 text-emerald-400' : 'text-gray-400 hover:text-white'}`}
            >
              Todos
            </button>
          </div>
          ...
          <button 
            ...
            className="bg-emerald-500 hover:bg-emerald-600 text-black font-bold p-3 rounded-xl flex items-center justify-center transition-all shadow-lg shadow-emerald-500/20"
          >
            <Plus size={20} />
          </button>
        </div>
        ...
        <div className="mb-8 glass-panel p-6 rounded-xl border border-emerald-500/30 animate-fade-in">
          ...
          <form onSubmit={handleSave} className="flex flex-col md:flex-row gap-4">
            <input 
              ...
              className="flex-1 bg-[#0B0C10] border border-white/10 rounded-lg p-3 text-white focus:border-emerald-500 outline-none"
              value={newName}
          ...
          return (
            <div key={client.id} className={`glass-panel p-5 rounded-xl border transition-all group flex flex-col relative ${overdue ? 'border-red-500/30 bg-red-500/5' : 'border-white/5 hover:border-emerald-500/30'}`}>
              ...
              <button 
                ...
                className="absolute top-2 right-2 p-3 text-gray-500 hover:text-red-500 hover:bg-red-500/10 rounded-full transition-all z-20 cursor-pointer"
              ...
              <div className="flex justify-between items-start mb-4 pr-10">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center text-white font-bold text-lg border border-white/10 group-hover:border-emerald-500 transition-colors">
                    {client.name.charAt(0)}
                  </div>
                  <div className="overflow-hidden">
                    <h4 className="text-white font-bold text-lg truncate w-full">{client.name}</h4>
               ...
               <div className="text-right mb-4">
                  <p className="text-xs text-gray-500 mb-1">Total Devido</p>
                  <p className={`font-mono text-xl font-bold ${client.balance > 0 ? (overdue ? 'text-red-400' : 'text-amber-400') : 'text-emerald-400'}`}>
              ...
              <div className="border-t border-white/5 pt-3 mt-auto space-y-3">
                <div className="flex items-center justify-between text-xs">
                   <span className="text-gray-500 flex items-center gap-1">
                    <Calendar size={12} /> Próx. Vencimento
                  </span>
                  <span className={`${overdue ? 'text-red-400 font-bold' : 'text-emerald-400 font-medium'}`}>
                    {client.balance > 0 ? dueDate : '-'}
                  </span>
                </div>
                ...
                {client.balance > 0 ? (
                  <div className="flex gap-2">
                    {/* Botão de Cobrar (WhatsApp) */}
                    <button 
                      onClick={() => handleBillClient(client)}
                      className={`flex-1 flex items-center justify-center gap-2 border py-3 rounded-lg transition-all text-sm font-bold ${overdue ? 'bg-red-500/10 border-red-500/50 text-red-400 hover:bg-red-500 hover:text-white' : 'bg-white/5 border-white/10 text-gray-300 hover:bg-white/10 hover:text-white'}`}
                      title="Enviar Cobrança via WhatsApp"
                    >
                      <MessageCircle size={16} />
                      {overdue ? 'Cobrar' : 'Lembrar'}
                    </button>
                    
                    {/* Botão de Receber (Dar Baixa) */}
                    <button 
                      onClick={() => handleSettleClick(client)}
                      className="flex-1 flex items-center justify-center gap-2 bg-emerald-500 text-black font-bold py-3 rounded-lg hover:bg-emerald-400 transition-all shadow-lg shadow-emerald-500/20"
                  ...
                  <div className="text-center py-2 text-sm text-gray-600 font-medium bg-white/5 rounded-lg border border-white/5 border-dashed">
                    Nada pendente
                  </div>
                )}
              </div>
          ...
          <div className="bg-[#1F2833] w-full max-w-sm rounded-2xl p-6 border border-white/10 shadow-2xl relative">
            ...
            <div className="flex flex-col items-center text-center mb-6">
              ...
              <p className="text-gray-400">
                ...
                <br/><span className="text-xs text-gray-500 mt-2 block">(O histórico de vendas permanecerá salvo)</span>
              </p>
            </div>
      ...
      {clientToSettle && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-[#1F2833] w-full max-w-sm rounded-2xl p-6 border border-white/10 shadow-2xl relative">
            <button 
              onClick={() => setClientToSettle(null)}
              className="absolute top-4 right-4 text-gray-500 hover:text-white"
            >
              <X size={24} />
            </button>
            ...
            <div className="flex flex-col items-center text-center mb-6">
              ...
              <p className="text-gray-400">
                Confirmar recebimento de <strong className="text-emerald-400">R$ {clientToSettle.balance.toFixed(2)}</strong> de <strong>{clientToSettle.name}</strong>?
                <br/><span className="text-xs text-gray-500 mt-2 block">(Isso zerará o saldo devedor do cliente)</span>
              </p>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button 
                ...
                className="flex-1 py-3 rounded-xl bg-emerald-500 text-black font-bold hover:bg-emerald-400 transition-colors shadow-lg shadow-emerald-500/20 flex items-center justify-center gap-2"
              >
                <Check size={18} />
                Confirmar
              </button>
